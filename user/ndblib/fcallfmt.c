                   ...
                   const void *const *args)
{
	...
	switch(type){
	case Tversion:	/* 100 */
		retval += fprintf(stream,"Tversion tag %ud msize %ud version '%s'", tag, f->msize, f->version);
		break;
	case Rversion:
		retval += fprintf(stream,"Rversion tag %ud msize %ud version '%s'", tag, f->msize, f->version);
		break;
	case Tauth:	/* 102 */
		retval += fprintf(stream,"Tauth tag %ud afid %d uname %s aname %s", tag,
	...
	case Rauth:
		...
		break;
	case Tattach:	/* 104 */
		retval += fprintf(stream,"Tattach tag %ud fid %d afid %d uname %s aname %s", tag,
	...
	case Rerror:	/* 107; 106 (Terror) illegal */
		...
		break;
	case Tflush:	/* 108 */
		retval += fprintf(stream,"Tflush tag %ud oldtag %ud", tag, f->oldtag);
		break;
	case Rflush:
		retval += fprintf(stream,"Rflush tag %ud", tag);
		break;
	case Twalk:	/* 110 */
		retval += fprintf(stream,"Twalk tag %ud fid %d newfid %d nwname %d ", tag, fid, f->newfid, f->nwname);
		if(f->nwname <= MAXWELEM)
			for(i=0; i<f->nwname; i++)
				retval += fprintf(stream, "%d:%s ", i, f->wname[i]);
		break;
	case Rwalk:
		retval += fprintf(stream,"Rwalk tag %ud nwqid %ud ", tag, f->nwqid);
		if(f->nwqid <= MAXWELEM)
			for(i=0; i<f->nwqid; i++){
				q = &f->wqid[i];
		...
		break;
	case Topen:	/* 112 */
		retval += fprintf(stream,"Topen tag %ud fid %ud mode %d", tag, fid, f->mode);
		break;
	case Ropen:
		retval += fprintf(stream,"Ropen tag %ud qid " QIDFMT " iounit %ud ", tag,
			f->qid.path, f->qid.vers, qidtype(tmp, f->qid.type), f->iounit);
		break;
	case Tcreate:	/* 114 */
		retval += fprintf(stream,"Tcreate tag %ud fid %ud name %s perm %d mode %d", tag,
		        fid, f->name, (uint32_t)f->perm, f->mode);
		break;
	case Rcreate:
		retval += fprintf(stream,"Rcreate tag %ud qid " QIDFMT " iounit %ud ", tag,
			f->qid.path, f->qid.vers, qidtype(tmp, f->qid.type), f->iounit);
		break;
	case Tread:	/* 116 */
		retval += fprintf(stream,"Tread tag %ud fid %d offset %lld count %ud",
			tag, fid, f->offset, f->count);
		break;
	case Rread:
		retval += fprintf(stream,"Rread tag %ud count %ud ", tag, f->count);
		retval += dumpsome(stream, f->data, f->count);
		break;
	case Twrite:	/* 118 */
		retval += fprintf(stream,"Twrite tag %ud fid %d offset %lld count %ud ",
			tag, fid, f->offset, f->count);
		retval += dumpsome(stream, f->data, f->count);
		break;
	case Rwrite:
		retval += fprintf(stream,"Rwrite tag %ud count %ud", tag, f->count);
		break;
	case Tclunk:	/* 120 */
