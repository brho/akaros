...
typedef struct Tdesc {			/* Legacy+Normal Transmit Descriptor */
	unsigned int	addr[2];
	unsigned int	control;		/* varies with descriptor type */
	unsigned int	status;			/* varies with descriptor type */
} Tdesc;
...
typedef struct ctlr {
	...
	/* bootstrap goo */
	struct block*	bqhead;	/* transmission queue */
	struct block*	bqtail;
} Ctlr;
...
static void
txstart(struct ether *edev)
{
	int tdh, tdt, len, olen;
	struct ctlr *ctlr = edev->ctlr;
	struct block *bp;
	Tdesc *tdesc;
	...
	for(tdt = ctlr->tdt; tdt != tdh; tdt = NEXT(tdt, Ntdesc)){
		...
		if((bp = ctlr->bqhead) == NULL)		/* was qget(edev->oq) */
			break;
		ctlr->bqhead = bp->next;
		if (ctlr->bqtail == bp)
			ctlr->bqtail = NULL;
		...
		if (len < ETHERMINTU) {
			if (bp->rp + ETHERMINTU <= bp->lim)
				...
				bp->wp = bp->lim;
			len = BLEN(bp);
			printd("txstart: extended short pkt %d -> %d bytes\n",
				olen, len);
		}

		/* set up a descriptor for it */
		tdesc = &ctlr->tdba[tdt];
		tdesc->addr[0] = (uintptr_t) /*PCIWADDR*/(bp->rp);
		tdesc->addr[1] = 0;
...
static struct block *
fromringbuf(struct ether *ether)
{
	RingBuf *tb = &ether->tb[ether->ti];
	struct block *bp = allocb(tb->len);
...
igbereplenish(struct ctlr* ctlr)
{
	...
	while(NEXT(rdt, Nrdesc) != ctlr->rdh){
		...
		else if((bp = iallocb(2048)) != NULL){
			ctlr->rb[rdt] = bp;
			rdesc->addr[0] = (uintptr_t) /*PCIWADDR*/(bp->rp);
			rdesc->addr[1] = 0;
		}
		else
			break;
...
static void
toringbuf(struct ether *ether, struct block *bp)
{
...
static void
igbeinterrupt((struct hw_trapframe *hw_tf, void* arg)
{
	struct block *bp;
	struct ctlr *ctlr;
	struct ether *edev;
	...
	if((icr = csr32r(ctlr, Icr)) & ctlr->im){
		...
		for(;;){
			...
			if ((rdesc->status & Reop) && rdesc->errors == 0) {
				...
				printd("igbe: input packet error 0x%ux\n",
					rdesc->errors);
			rdesc->status = 0;
			rdh = NEXT(rdh, Nrdesc);
		}
...
igbeinit(struct ether* edev)
{
	...
	csr32w(ctlr, Rdbal, (uintptr_t) /*PCIWADDR*/(ctlr->rdba));
	csr32w(ctlr, Rdbah, 0);
	...
	ctlr->rb = kzmalloc(sizeof(struct block *) * Nrdesc, 0);
	igbereplenish(ctlr);
	...
	csr32w(ctlr, Tdbal, (uintptr_t) /*PCIWADDR*/(ctlr->tdba));
	csr32w(ctlr, Tdbah, 0);
	...
	ctlr->tb = kzmalloc(sizeof(struct block *) * Ntdesc, 0);
//	ctlr->im |= Txqe|Txdw;
...
{
	struct miiphy *phy = (struct miiphy *)1;
	int ctrl, p, r;
	...
	if(mii(ctlr->mii, ~0) == 0 || (phy = ctlr->mii->curphy) == NULL){
		if (0)
			printd("phy trouble: phy = 0x%lux\n", (uint32_t)phy);
		kfree(ctlr->mii);
		ctlr->mii = NULL;
		return -1;
	}
...
#warning "Barret! save me!"
	...
	while(p = pcimatch(p, 0x8086, 0)){
		...
		if(port == 0){
			printd("igbe: can't map %d @ 0x%8.8luX\n",
		...
		switch(cls){
			default:
				printd("igbe: unexpected CLS - %d bytes\n",
					cls*sizeof(long));
				break;
			case 0x00:
			case 0xFF:
				/* alphapc 164lx returns 0 */
				printd("igbe: unusable PciCLS: %d, using %d longs\n",
