...
static void
addqueue(struct route **q, struct route *r)
{
	struct route *l;
...
static int
rangecompare(struct route *a, struct route *b)
{
...
static void
copygate(struct route *old, struct route *new)
{
 ...
 */
static void
walkadd(struct fs *f, struct route **root, struct route *p)
{
	struct route *l, *r;
 ...
 */
static void
addnode(struct fs *f, struct route **cur, struct route *new)
{
	struct route *p;
...
struct route**
looknode(struct route **cur, struct route *r)
{
	struct route *p;
...
void
v4delroute(struct fs *f, uint8_t *a, uint8_t *mask, int dolock)
{
	struct route **r, *p;
	struct route rt;
	int h, eh;
...
void
v6delroute(struct fs *f, uint8_t *a, uint8_t *mask, int dolock)
{
	struct route **r, *p;
	struct route rt;
	int h, eh;
...
struct route*
v4lookup(struct fs *f, uint8_t *a, struct conv *c)
{
	struct route *p, *q;
	uint32_t la;
	...
	if(c != NULL && c->r != NULL && c->r->routeTree.ifc != NULL && c->rgen == v4routegeneration)
		return c->r;
	...
	if(q && (q->routeTree.ifc == NULL || q->routeTree.ifcid != q->routeTree.ifc->ifcid)){
...
struct route*
v6lookup(struct fs *f, uint8_t *a, struct conv *c)
{
	struct route *p, *q;
	uint32_t la[IPllen];
	int h;
	...
	if(c != NULL && c->r != NULL && c->r->routeTree.ifc != NULL && c->rgen == v6routegeneration)
		return c->r;
 ...
 */
static void
sprintroute(struct route *r, struct routewalk *rw)
{
	int nifc, n;
 ...
 */
static int
rr(struct route *r, struct routewalk *rw)
{
	int h;
...
void
ipwalkroutes(struct fs *f, struct routewalk *rw)
{
	rlock(&routelock);
	if(rw->e > rw->p) {
...
long
routeread(struct fs *f, char *p, uint32_t offset, int n)
{
	struct routewalk rw;
 ...
 */
void
delroute(struct fs *f, struct route *r, int dolock)
{
 ...
 */
int
routeflush(struct fs *f, struct route *r, char *tag)
{
	if(r == NULL)
		return 0;
	...
	if((r->routeTree.type & Rifc) == 0){
		if(tag == NULL || strncmp(tag, r->routeTree.tag, sizeof(r->routeTree.tag)) == 0){
			delroute(f, r, 0);
			return 1;
		}
	}
...
struct route *
iproute(struct fs *fs, uint8_t *ip)
{
	if(isv4(ip))
