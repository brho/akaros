...
void
mkqid(struct qid *q, int64_t path, uint32_t vers, int type, struct errbuf *perrbuf)
{
	q->type = type;
	q->vers = vers;
	q->path = path;
...
void
devdir(struct chan *c, struct qid qid, char *n, int64_t length, char *user, long perm, struct dir *db)
{
	db->name = n;
	if(c->flag&CMSG)
		qid.type |= QTMOUNT;
...
struct chan*
devattach(int dc, char *spec, struct errbuf *perrbuf)
{
	struct chan *c;
	char *buf;
...
struct chan*
devclone(struct chan *c, struct errbuf *perrbuf)
{
	struct chan *nc;
...
long
devstat(struct chan *c, uint8_t *db, long n, struct dirtab *tab, int ntab, devgen_t *gen, struct errbuf *perrbuf)
{
	int i;
	struct dir dir;
	char *p, *elem;
...
long
devdirread(struct chan *c, char *d, long n, struct dirtab *tab, int ntab, devgen_t *gen, struct errbuf *perrbuf)
{
	long m, dsz;
	struct dir dir;
